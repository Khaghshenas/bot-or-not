...
try:
    _nht_class_index = list(pipeline.classes_).index(
        NHT_CLASS_LABEL
    )
...
def predict_traffic(log_line: LogLine) -> dict:
    ...
    # Run preprocessing and model inference once, then derive the class prediction from the returned probabilities.
