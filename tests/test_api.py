...
def test_prediction_contract():
    ...
    assert "X-Prediction-Time-Ms" in response.headers
    assert float(response.headers["X-Prediction-Time-Ms"]) >= 0
    assert response.headers["X-Model-Version"] == APP_VERSION
